...
**fundamental programming concepts** using the **.NET framework** [1, 2]. The projects
...
complex academic analytics [4, 5].
* **Logical Architecture:** Uses a **Counter-Controlled loop** (`For...Next`) to manage
multiple student entries and a **Pretest loop** (`Do While`) for input validation [5, 6].
...
precision during user input processing [10, 11].
...
(RAD)** strategy, focusing on university branding and student financial management [12,
13].
* **User Interface Design:**
* **Branding:** Features a **PictureBox** utilizing `StretchImage` mode to accommodate
the high-resolution KAAF logo (2274px x 1113px) [14-16].
* **Layout:** Uses **GroupBox** containers to organize &quot;Personal Information&quot; and
&quot;Residency Status,&quot; creating an intuitive flow [17, 18].
* **Controls:** Integrated **ComboBox** for course selection, **CheckBoxes** for
optional fees, and **RadioButtons** for mutually exclusive residency choices [17, 19, 20].
* **Logic &amp; Validation:**
* **Event-Driven Programming:** Operations are triggered by the `Click` event of the
processing button [3, 21].
* **Robustness:** Uses **Short-circuiting logical operators** (`AndAlso`, `OrElse`) and
`TryParse` methods to prevent runtime errors during financial calculations [22-24].
...
* **Framework:** .NET 10 [31]
* **IDE:** Visual Studio 2026 [32]
* **Execution:**
1. Open the `.sln` file in Visual Studio.
2. Press **Ctrl+F5** to run the project without debugging [31].# KayasaVbasicMidSemProject2
